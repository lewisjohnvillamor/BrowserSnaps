...
Uses Chrome DevTools Protocol only during a user-started capture to apply selected responsive viewport dimensions, capture visible viewport tiles in the optional dedicated capture window, and read the sizes and headers of the page's own network responses, throttle the connection and record a screencast filmstrip of the page's own load for the optional local performance score. It attaches only to the selected capture tab and detaches when the capture finishes, fails, or is cancelled.
