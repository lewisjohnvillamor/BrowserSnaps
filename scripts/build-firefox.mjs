...
const manifest = JSON.parse(await readFile(path.join(root, "manifest.firefox.json"), "utf8"));
const packageJson = JSON.parse(await readFile(path.join(root, "package.json"), "utf8"));

if (manifest.version !== packageJson.version) {
  throw new Error(`Version mismatch: Firefox manifest ${manifest.version}, package ${packageJson.version}`);
}

const dist = path.join(root, "dist");
const staging = path.join(dist, "firefox-extension");
const archive = path.join(dist, `BrowserSnaps-v${manifest.version}-firefox.zip`);
...
await cp(path.join(root, "manifest.firefox.json"), path.join(staging, "manifest.json"));
